 ...
 */
import type { Mpc, MpcHead, MpcFrame } from "./types";
import { getLittleEndianInt } from "./utils";
...
/**
 * Cache for loaded MPC files
 */
const mpcCache = new Map<string, Mpc | null>();
...
export async function loadMpc(url: string): Promise<Mpc | null> {
  if (mpcCache.has(url)) {
    return mpcCache.get(url) ?? null;
  }

  try {
    const response = await fetch(url);
    if (!response.ok) {
      console.warn(`Failed to load MPC: ${url}`);
      mpcCache.set(url, null);
      return null;
    }
    const buffer = await response.arrayBuffer();
    const mpc = await parseMpc(buffer);
    mpcCache.set(url, mpc);
    return mpc;
  } catch (error) {
    console.warn(`Error loading MPC ${url}:`, error);
    mpcCache.set(url, null);
    return null;
  }
}
