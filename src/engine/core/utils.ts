...
// === BezierCurve (C#: Engine/Lib/BezierCurve.cs) ===
// 贝塞尔曲线计算，用于角色跳跃移动
...
function bernstein(n: number, i: number, t: number): number {
  // t^i
  const ti = t === 0.0 && i === 0 ? 1.0 : Math.pow(t, i);
  // (1 - t)^(n-i)
  const tni = n === i && t === 1.0 ? 1.0 : Math.pow(1 - t, n - i);
