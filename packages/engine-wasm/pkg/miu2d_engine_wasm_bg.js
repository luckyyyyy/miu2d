 ...
 * 寻路器状态（可复用以减少内存分配）
 *
 * 使用共享内存模式：JS 端创建 SharedArrayBuffer，Rust 只读访问
 * 这避免了跨边界的数据拷贝，大幅提升性能
 */
export class PathFinder {
    ...
    /**
     * 获取位图大小（字节数），用于 JS 端分配共享内存
     * @returns {number}
     */
    get_bitmap_size() {
        const ret = wasm.pathfinder_get_bitmap_size(this.__wbg_ptr);
        return ret >>> 0;
    }
    /**
     * 检查共享内存是否已设置
     ...
     */
    is_memory_ready() {
        const ret = wasm.pathfinder_is_memory_ready(this.__wbg_ptr);
        return ret !== 0;
    }
    /**
     * 创建新的寻路器
     * 注意：必须随后调用 set_shared_memory 设置共享内存指针
     ...
     * # Safety
     * 调用者必须确保：
     * 1. 指针指向有效的 SharedArrayBuffer 内存
     * 2. 内存大小至少为 bitmap_size 字节
     * 3. 在 PathFinder 生命周期内内存保持有效
     * @param {number} obstacle_ptr
     * @param {number} hard_obstacle_ptr
     */
    set_shared_memory(obstacle_ptr, hard_obstacle_ptr) {
        wasm.pathfinder_set_shared_memory(this.__wbg_ptr, obstacle_ptr, hard_obstacle_ptr);
    }
}
