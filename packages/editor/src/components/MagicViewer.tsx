...
function FormRow({
  ...
  return (
    <div className="flex items-center gap-2">
      <label
        className="w-28 shrink-0 text-xs text-[#808080] truncate"
        title={tooltip || label}
      >
        {label}
      </label>
...
function LevelTable({
  ...
  levels: Map<number, Partial<MagicData>> | undefined;
}) {
  if (!levels || levels.size === 0) {
    return (
      <div className="text-center text-[#808080] py-4 text-sm">
        无等级数据
      </div>
    );
  }
      ...
      <table className="w-full text-xs border-collapse">
        <thead>
          <tr className="bg-[#2d2d2d] text-[#808080]">
            <th className="border border-[#3c3c3c] px-2 py-1 text-left">
              等级
            </th>
            <th className="border border-[#3c3c3c] px-2 py-1 text-right">
              效果值
            </th>
            <th className="border border-[#3c3c3c] px-2 py-1 text-right">
              法力消耗
        ...
        <tbody>
          {levelArray.map(([level, data]) => (
            <tr
              key={level}
              className="text-[#cccccc] hover:bg-[#2a2a2a]"
            >
...
export function MagicViewer({
  ...
  // 当前激活的标签页
  const [activeTab, setActiveTab] = useState<"config" | "levels" | "json">(
    "config"
  );
  ...
  const moveKindOptions = Object.entries(MOVE_KIND_LABELS).map(
    ([value, label]) => ({
      value: Number(value),
      label: `${value} - ${label}`,
    })
  ...
  const specialKindOptions = Object.entries(SPECIAL_KIND_LABELS).map(
    ([value, label]) => ({
      value: Number(value),
      label: `${value} - ${label}`,
    })
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-[#3c3c3c] bg-[#252526]">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            {fileName && (
              <p className="text-xs text-[#808080]">{fileName}</p>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2">
        ...
        {activeTab === "config" && (
          <div className="max-w-2xl">
            ...
            <ConfigSection title="资源文件" icon="🎨" defaultOpen={true}>
              ...
              <FormRow
                label="飞行动画"
                value={magic.flyingImage}
                readOnly={readOnly}
              />
              <FormRow
                label="飞行音效"
                value={magic.flyingSound}
                readOnly={readOnly}
              />
              <FormRow
                label="消失动画"
                value={magic.vanishImage}
                readOnly={readOnly}
              />
              <FormRow
                label="消失音效"
                value={magic.vanishSound}
                readOnly={readOnly}
              />
              <FormRow
                label="超级模式动画"
                value={magic.superModeImage}
                readOnly={readOnly}
              />
            </ConfigSection>
            ...
            <ConfigSection title="移动参数" icon="🚀" defaultOpen={true}>
              ...
              <FormRow
                label="等待帧数"
                value={magic.waitFrame}
                type="number"
                readOnly={readOnly}
              />
              <FormRow
                label="生命帧数"
                value={magic.lifeFrame}
                type="number"
                readOnly={readOnly}
              />
            </ConfigSection>
            ...
            <ConfigSection title="渲染参数" icon="✨" defaultOpen={false}>
              ...
              <FormRow
                label="飞行亮度"
                value={magic.flyingLum}
                type="number"
                readOnly={readOnly}
              />
              <FormRow
                label="消失亮度"
                value={magic.vanishLum}
                type="number"
                readOnly={readOnly}
              />
            </ConfigSection>
            ...
            <ConfigSection title="效果值" icon="💪" defaultOpen={false}>
              ...
              <FormRow
                label="法力消耗"
                value={magic.manaCost}
                type="number"
                readOnly={readOnly}
              />
              <FormRow
                label="体力消耗"
                value={magic.thewCost}
                type="number"
                readOnly={readOnly}
              />
            </ConfigSection>
            ...
            <ConfigSection title="关联武功" icon="🔗" defaultOpen={false}>
              ...
              <FormRow
                label="动作文件"
                value={magic.actionFile}
                readOnly={readOnly}
              />
              <FormRow
                label="攻击文件"
                value={magic.attackFile}
                readOnly={readOnly}
              />
              <FormRow
                label="第二武功"
                value={magic.secondMagicFile}
                readOnly={readOnly}
              />
              <FormRow
                label="爆炸武功"
                value={magic.explodeMagicFile}
                readOnly={readOnly}
              />
            </ConfigSection>
