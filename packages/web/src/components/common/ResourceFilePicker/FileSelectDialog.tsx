...
import type { FileTreeNode, FlatFileTreeNode, ExpandedState } from "../../../pages/dashboard/modules/fileTree/types";
...
export function FileSelectDialog({
  ...
  // 悬停预览
  const [hoverNode, setHoverNode] = useState<{ node: FlatFileTreeNode; position: { x: number; y: number } } | null>(null);
  // 搜索
  ...
  // 展开/折叠
  const handleToggle = useCallback(async (node: FlatFileTreeNode) => {
  ...
  // 双击选择
  const handleDoubleClick = useCallback((node: FlatFileTreeNode) => {
    if (node.isDirectory) {
      handleToggle(node);
      return;
    }
  ...
  // 悬停预览
  const handleMouseEnter = useCallback((e: React.MouseEvent, node: FlatFileTreeNode) => {
  ...
  useEffect(() => {
    ...
    expandToPath();
  }, [open, hasInitialized, currentPath, rootQuery.data, gameId, utils.file.list, expandedState, loadedDirs]);
