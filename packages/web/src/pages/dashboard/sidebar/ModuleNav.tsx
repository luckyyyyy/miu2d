...
const ACCENTS: Record<AccentColor, { color: string; bg: string; glow: string }> = {
  ...
  violet:  { color: "#a78bfa", bg: "rgba(167,139,250,0.10)", glow: "0 0 12px rgba(167,139,250,0.12)" },
  emerald: { color: "#34d399", bg: "rgba(52,211,153,0.10)",  glow: "0 0 12px rgba(52,211,153,0.12)" },
...
/** 分组内的子项（无图标，紧凑样式） */
function SectionNavItem({ item, basePath, accent }: {
...
export function SectionedModuleNav({ sections, basePath }: {
          ...
          <div key={section.label} className="mb-1">
            ...
            <div className="flex flex-col gap-0.5">
              {section.items.map((item) => (
                <SectionNavItem key={item.id} item={item} basePath={basePath} accent={section.accent} />
              ))}
            </div>
          </div>
        );
