            ...
            {games.length === 0 ? (
              ...
              games.map((game) => (
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-2 text-left hover:bg-[#2a2d2e] transition-colors ${
                ...
                >
                  ...
                  {currentGame?.id === game.id && (
                    <span className="text-[#0098ff] text-xs">当前</span>
                  )}
                </button>
              ))
            )}
 ...
 * 带数据获取的游戏选择器包装组件
 */
export function GameSelectorWithData({
  onCreateGame,
}: {
  onCreateGame?: () => void;
}) {
