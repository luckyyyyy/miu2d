...
export function MapDataPanel({
  ...
  const [hoverMapScript, setHoverMapScript] = useState<{ name: string; position: { x: number; y: number } } | null>(null);
  ...
  const availableScripts = useMemo(() => {
    ...
    if (sceneData.traps) {
      for (const key of Object.keys(sceneData.traps)) scripts.push(key);
    }
    if (sceneData.scripts) {
      for (const key of Object.keys(sceneData.scripts)) {
        if (!scripts.includes(key)) scripts.push(key);
      }
    }
    return scripts.sort();
  ...
  return (
    <div className="flex flex-col gap-3 p-3 text-sm overflow-auto">
      ...
      <div className="flex flex-col gap-1 text-zinc-400 text-xs">
        <div>
          尺寸: {mapData.mapColumnCounts} × {mapData.mapRowCounts} 瓦片
          ({mapData.mapPixelWidth} × {mapData.mapPixelHeight} px)
        </div>
          ...
          <div className="flex flex-col gap-1">
            {unifiedTraps.map((entry) => {
              const hasFile = !!(
                ...
                entry.scriptPath &&
                sceneData.traps &&
                entry.scriptPath in sceneData.traps
              );
              return (
                <div
                ...
                >
                  ...
                  {entry.orphan ? (
                    <>
                      ...
                      <span className="shrink-0 text-[10px] text-zinc-500">{entry.tileCount}格</span>
                      <button
                        type="button"
                      ...
                      <button
                        ...
                        onClick={() => setConfirmDelete({ idx: -1, mode: "orphan", trapIndex: entry.trapIndex })}
                      >
                        ✕
                      </button>                    </>
                  ) : (
                    <>
                      ...
                      >
                        {!hasFile && <span className="text-yellow-500 mr-1" title="脚本文件不存在">⚠</span>}
      ...
      <FileSelectDialog
        ...
        currentValue={scriptPickerIdx !== null ? mapData.trapTable[scriptPickerIdx]?.scriptPath : undefined}
        extensions={[".txt"]}
        title="选择脚本"
        headerExtra={
            ...
            <>
                    ...
                    {filteredMapScripts.map((name) => {
                      return (
                        <div
                          ...
                          onMouseEnter={(e) => setHoverMapScript({ name, position: { x: e.clientX, y: e.clientY } })}
              ...
              <div className="flex items-center justify-between px-4 py-3 border-t border-[#454545] bg-[#252526]">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => { if (selectedMapScript) handleScriptSelect(selectedMapScript); }}
      ...
      {/* 删除确认弹窗 */}
      {confirmDelete && (confirmDelete.mode === "orphan" || mapData.trapTable[confirmDelete.idx]) && (
        <ConfirmDialog
