...
import { createDefaultNpcEntry, createDefaultObjEntry, ITEM_HEIGHT, OVERSCAN } from "./scene-constants";
...
export function SceneItemEditorPanel({
  ...
  // NPC/OBJ 结构化条目
  const entries = isStructured
    ? (kind === "npc" ? ctx.npcEntries : ctx.objEntries)
    : [];
  ...
  // 脚本/陷阱文本内容
  const textContent = kind === "script" ? ctx.scriptContent : kind === "trap" ? ctx.trapContent : "";
  const setTextContent = kind === "script" ? ctx.setScriptContent : ctx.setTrapContent;

  // dirty & markSaved — 四种类型统一分派
  const isDirty =
    kind === "npc" ? ctx.npcDirty
    : kind === "obj" ? ctx.objDirty
    : kind === "script" ? ctx.scriptDirty
    : ctx.trapDirty;

  const markSaved =
    kind === "npc" ? ctx.markNpcSaved
    : kind === "obj" ? ctx.markObjSaved
    : kind === "script" ? ctx.markScriptSaved
    : ctx.markTrapSaved;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (isStructured) {
        ...
        newData[field] = { ...(sceneData[field] ?? {}), [itemKey]: textContent };
      }
  ...
  const getItemTop = useCallback(
    (i: number) => {
      if (selectedIdx === null || selectedIdx === undefined || selectedIdx < 0 || selectedIdx >= entries.length) {
        return i * ITEM_HEIGHT;
      }
      if (i <= selectedIdx) return i * ITEM_HEIGHT;
      return selectedIdx * ITEM_HEIGHT + (expandedHeight || ITEM_HEIGHT) + (i - selectedIdx - 1) * ITEM_HEIGHT;
    },
