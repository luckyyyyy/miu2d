...
export function ScriptFieldPicker({
  ...
  const [hoverScript, setHoverScript] = useState<{ name: string; position: { x: number; y: number } } | null>(null);
      ...
      <>
            ...
            <div className="space-y-0.5">
              {filteredMapScripts.map((name) => (
                <div
                  ...
                  onMouseEnter={(e) => setHoverScript({ name, position: { x: e.clientX, y: e.clientY } })}
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-[#454545] bg-[#252526]">
          <div className="text-sm text-[#808080]">
            {selectedMapScript ? (
              <span className="truncate max-w-80 inline-block" title={selectedMapScript}>{selectedMapScript}</span>
            ) : "未选择文件"}
          </div>
          <div className="flex gap-2">
            ...
            <button
              type="button"
              onClick={() => { if (selectedMapScript) handleSelect(selectedMapScript); }}
  ...
  return (
    <div className="space-y-1.5">
      ...
      <div className={fieldBoxCls}>
        ...
        <NumInput value={entry.walkSpeed} onChange={(v) => onChange("walkSpeed", v)} title="移动速度" />
      ...
      <div className={fieldBoxCls}>
        ...
        <NumInput value={entry.attackLevel} onChange={(v) => onChange("attackLevel", v)} title="攻击等级" />
        <span className={labelTagCls}>攻距</span>
        <NumInput value={entry.attackRadius} onChange={(v) => onChange("attackRadius", v)} title="攻击半径" />
      </div>
      <FieldRow label="尸体">
        <input className={inputCls} value={entry.bodyIni} onChange={(e) => onChange("bodyIni", e.target.value)} />
      </FieldRow>
      <FieldRow label="武功">
        <input className={inputCls} value={entry.flyIni} onChange={(e) => onChange("flyIni", e.target.value)} />
      </FieldRow>
      ...
      <div className={fieldBoxCls}>
        ...
        <NumInput value={entry.levelUpExp} onChange={(v) => onChange("levelUpExp", v)} title="升级所需经验" />
      </div>
      ...
      {entry.fixedPos ? (
        <>
          ...
          <FieldRow label="FixedPos">
            <input className={inputCls} value={entry.fixedPos} onChange={(e) => onChange("fixedPos", e.target.value)} />
          </FieldRow>
        </>
      ) : null}
    </div>
