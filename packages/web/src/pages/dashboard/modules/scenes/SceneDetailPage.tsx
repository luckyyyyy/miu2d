...
function SceneDetailContent() {
  ...
  const [trapInputDialog, setTrapInputDialog] = useState<{ open: boolean; value: string; tileX: number; tileY: number } | null>(null);
  /** 自定义障碍号码输入弹窗（保存 tileX/tileY 以避免依赖 mapContextMenu） */
  const [barrierInputDialog, setBarrierInputDialog] = useState<{ open: boolean; value: string; tileX: number; tileY: number } | null>(null);
  ...
  const handleTrapTileClick = useCallback(
    (trapIndex: number) => {
      ...
      setSearchParams((prev) => {
        ...
        next.set("trapKey", entry.scriptPath);
        // 清除旧的 kind/key 参数
        next.delete("kind");
        next.delete("key");
        return next;
    ...
    (entity: NpcListItem | ObjListItem) => {
      ...
      if (kind === "npc") {
        ...
        const newEntry: SceneNpcEntry = {
          ...
          relation: NpcRelationValues[(npc as NpcListItem).relation as keyof typeof NpcRelationValues] ?? 0,
          mapX,
          mapY,
        };
        setNpcEntries((prev) => [...prev, newEntry]);
  ...
  const handleContextMenuCreate = useCallback(
    (kind: "trap" | "npc" | "obj", specifiedTrapIndex?: number, overrideTile?: { tileX: number; tileY: number }) => {
      ...
      if (kind === "trap") {
        ...
        const trapIndex = specifiedTrapIndex ?? (() => {
          let next = 1;
  ...
  const handleMapPanelTrapSelect = useCallback(
    (trapIndex: number) => {
      setPanelHighlightTraps([trapIndex]);
    },
    [],
    ...
    <div className="flex h-full">
                ...
                children: [
                  {
                    ...
                    onClick: () => setTrapInputDialog({ open: true, value: customTrapIndex || "1", tileX, tileY }),
                  },
                  { label: "", divider: true, onClick: () => {} },
                ...
                children: [
                  {
                    ...
                    onClick: () => setBarrierInputDialog({ open: true, value: customBarrierValue || "128", tileX, tileY }),
                  },
                  { label: "", divider: true, onClick: () => {} },
      ...
      {trapInputDialog && (
        <div
          ...
          onClick={(e) => { if (e.target === e.currentTarget) setTrapInputDialog(null); }}
        >
      ...
      {barrierInputDialog && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-black/50"
          onClick={(e) => { if (e.target === e.currentTarget) setBarrierInputDialog(null); }}
        >
          <div className="bg-[#252526] border border-[#555] rounded-lg shadow-xl w-72 p-4">
            <h3 className="text-sm text-zinc-200 font-medium mb-3">输入障碍值 (十进制, 如 128=0x80)</h3>
            <input
              autoFocus
              type="number"
              min={1}
              ...
              onChange={(e) => setBarrierInputDialog({ ...barrierInputDialog, value: e.target.value })}
              onKeyDown={(e) => {
                if (e.key === "Enter") {
