...
export function MagicPreview({ gameSlug, magic, level = 1 }: MagicPreviewProps) {
    ...
    (centerX: number, centerY: number, targetDirection8: number): FlyingSprite[] => {
      ...
      switch (moveKindNum) {
        ...
        case 5: { // HeartMove - 心形移动（时序展开）
          ...
          // Final
          sprites.push(createSpriteFromDir(directions32[0], centerX, centerY, (18 + 15) * delayTime));
          break;
        }

        case 6: { // SpiralMove - 螺旋移动（时序展开）
          const magicDelayMs = 30;
          for (let i = 0; i < 32; i++) {
            const dirIdx = (dir32Index + i) % 32;
            sprites.push(createSpriteFromDir(directions32[dirIdx], centerX, centerY, i * magicDelayMs));
          }
          break;
        }

        case 7: { // SectorMove - 扇形移动（复用引擎 addSectorMoveMagicSprite）
          let count = 1;
          if (effectLevel > 0) {
        ...
        case 9: { // FixedWall - 固定墙（在目标位置形成一排）
          const targetX = centerX + targetDir.x * PREVIEW_DISTANCE * 0.6;
          const targetY = centerY + targetDir.y * PREVIEW_DISTANCE * 0.6;
        ...
        case 10: { // WallMove - 移动墙（墙向前移动）
          ...
          // 两侧
          for (let i = 1; i <= halfCount; i++) {
            sprites.push(createSpriteFromDir(targetDir, centerX + wallOffset.x * i, centerY + wallOffset.y * i));
            sprites.push(createSpriteFromDir(targetDir, centerX - wallOffset.x * i, centerY - wallOffset.y * i));
          }
          break;
        }

        case 11: { // RegionBased - 根据 Region 值决定区域类型
          ...
          switch (region) {
              ...
              for (let row = 0; row < rowCount; row++) {
                for (let col = 0; col < rowCount; col++) {
                  ...
                  sprites.push({
                    ...
                    x, y, vx: 0, vy: 0,
                    direction: targetDirection8,
                    frame: 0, phase: "flying",
                    delayMs: 0, elapsedMs: 0,
                  });
                }
              }
              break;
              ...
              for (let i = 0; i < rowCount; i++) {
                ...
                for (const offset of crossOffsets) {
                  sprites.push({
                    ...
                    vx: 0, vy: 0,
                    direction: targetDirection8,
                    frame: 0, phase: "waiting",
                    delayMs: delay, elapsedMs: 0,
                  });
                }
              }
              break;
            }
            case 3: { // 矩形区域 (Rectangle) - 推山填海使用
              // 根据方向创建多排向前推进的火墙
            ...
            case 4: { // 等腰三角形 (Isosceles Triangle)
              for (let row = 0; row < rowCount; row++) {
                const delay = row * magicDelayMs;
        ...
        case 24: { // VMove - V字移动（使用引擎 getVOffsets）
          // 获取当前方向对应的 V 字偏移
          const vOffsets = getVOffsets(targetDirection8);
          // 中心武功
          sprites.push(createSpriteFromDir(targetDir));
  ...
  useEffect(() => {
    ...
    const animate = (time: number) => {
      ...
      for (const sprite of spritesRef.current) {
        ...
        if (sprite.phase === "flying") {
          ...
          const effectiveInterval = (isSuperMode && sprite.isSuperModeCast && superModeAsf)
          ...
          const distFromCenter = Math.sqrt(
            (sprite.x - centerX) ** 2 + (sprite.y - centerY) ** 2
          );

          // SuperMode 施法精灵使用 superModeAsf 的帧数
          const effectiveFramesPerDir = (isSuperMode && sprite.isSuperModeCast && superModeAsf)
    ...
    const drawFrame = (ctx: CanvasRenderingContext2D, width: number, height: number, isSuperMode: boolean) => {
      // 清空画布
      ...
      // 获取亮度和透明混合参数
      const alphaBlend = magic?.alphaBlend || false;
  ...
  }, [isPlaying, isLoading, flyingAsf, vanishAsf, superModeAsf, magic, resetSimulation, currentDirection, level]);
...
function TrajectoryInfo({ moveKind, speed }: { moveKind: MagicMoveKind; speed?: number }) {
  ...
  const getDescription = (): string => {
    switch (moveKindNum) {
      case 0: return "原地释放，无飞行阶段";
      case 1: return "固定位置产生效果";
      case 2: return "单发直线飞行";
      case 3: return "多发直线散射";
      case 4: return "圆形向四周扩散";
      case 5: return "沿心形轨迹移动";
      case 6: return "螺旋形向外扩散";
      case 7: return "扇形范围攻击";
      case 8: return "随机扇形散射";
      case 13: return "跟随施法者移动";
      case 15: return "全屏攻击所有目标";
      case 16: return "追踪最近敌人";
      case 20: return "传送到目标位置";
      case 22: return "在目标位置召唤 NPC";
      case 24: return "V 字形双向飞行";
      default: return "未知移动类型";
    }
  };
  ...
  return (
    <div className="bg-[#252525] rounded p-3 text-sm">
      ...
      {speed !== undefined && (
        <p className="text-[#858585] text-xs mt-1">飞行速度: {speed}</p>
      )}
    </div>
  );
}
