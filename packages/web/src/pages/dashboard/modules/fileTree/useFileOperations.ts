...
interface UseFileOperationsOptions {
  ...
  updateNodeInTree: (nodes: FileTreeNode[], id: string, updater: (n: FileTreeNode) => FileTreeNode) => FileTreeNode[];
  removeNodeFromTree: (nodes: FileTreeNode[], id: string) => FileTreeNode[];
  insertNodeInTree: (nodes: FileTreeNode[], parentId: string | null, node: FileTreeNode) => FileTreeNode[];
...
export function useFileOperations({
  ...
  const renameNode = useCallback(
    async (node: FlatFileTreeNode, newName: string) => {
      ...
      try {
        ...
        // 回滚
        setTreeNodes((prev) =>
          updateNodeInTree(prev, node.id, (n) => ({ ...n, name: oldName })),
        );
      }
    },
      ...
      try {
        ...
        if (gameId) {
          utils.file.list.setData(
            { gameId, parentId: node.parentId },
            (old) => (old ? old.filter((item) => item.id !== node.id) : old),
          );
        }
      } catch {
        // 回滚
  ...
  const moveNode = useCallback(
    async (nodeId: string, newParentId: string | null) => {
      ...
      setTreeNodes((prev) => {
        // 找到节点
        const findNode = (nodes: FileTreeNode[], parentId: string | null): [FileTreeNode | null, string | null] => {
