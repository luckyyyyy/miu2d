    ...
    <div className="h-screen w-screen flex items-center justify-center bg-[#1e1e1e]">
      <div className="text-center max-w-md">
        ...
        <p className="text-[#858585] mb-2">
          找不到名为 <code className="bg-[#2d2d2d] px-2 py-1 rounded text-[#ce9178]">{slug}</code> 的游戏空间
        </p>
        <p className="text-[#858585] mb-8">
          请检查链接是否正确，或者该空间可能已被删除
        </p>
        <a
          href="/dashboard"
...
function GameContextSetter({ children }: { children: React.ReactNode }) {
  ...
  const { data: game, isLoading, isError } = trpc.game.getBySlug.useQuery(
    { slug: gameSlug! },
    { enabled: !!gameSlug }
  );
