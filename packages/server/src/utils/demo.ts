...
import { games, gameMembers, users } from "../db/schema";
...
export async function seedDemoData(): Promise<void> {
	...
	if (!demoGame) {
		[demoGame] = await db.insert(games).values({
			slug: DEMO_SLUG,
			name: "Demo Game",
	...
	const [existingMember] = await db
		...
		.where(
			and(
				eq(gameMembers.gameId, demoGame.id),
				eq(gameMembers.userId, DEMO_DEV_USER_ID),
			),
		)
		.limit(1);
